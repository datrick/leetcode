	...
	private boolean checkMatch(String s, int i, String p, int j){
		...
		if (s.charAt(i) == '*'){
			if (i == s.length() - 1)
				return true;
			if (p.charAt(j) == '*')
				return this.checkMatch(s, i, p, j + 1) || this.checkMatch(s, i + 1, p, j);
			do {
...
//		String[] sArray = {"baaabbabbbaabbbbbbabbbaaabbaabbbbbaaaabbbbbabaaaaabbabbaabaaababaabaaabaaaabbabbbaabbbbbaababbbabaaabaabaaabbbaababaaabaaabaaaabbabaabbbabababbbbabbaaababbabbaabbaabbbbabaaabbababbabababbaabaabbaaabbba"}, pArray = {"*b*ab*bb***abba*a**ab***b*aaa*a*b****a*b*bb**b**ab*ba**bb*bb*baab****bab*bbb**a*a*aab*b****b**ba**abba"};	// 
//		String[] pArray = {"***"}, sArray = {"abaabaabbbabbaabbbbbbabbabbababab"};
		for (int i = 0; i < sArray.length && i < pArray.length; i ++)
			System.out.println(new SolutionII().isMatch(sArray[i], pArray[i]));
	}
}
