...
public class Solution {
	...
	public int[] maxNumber(int[] nums1, int[] nums2, int k) {
		int[] res = new int[k];
		Deque<Integer> deque = new LinkedList<>(), next = null;
		deque.offer(0);
		deque.offer(0);
		int i = 0, len = k;
		while (i < len) {
			...
			int idx1 = 0, idx2 = 0;
			while (! deque.isEmpty()) {
				int j = deque.poll(), l = deque.poll();
				idx1 = j;
				idx2 = l;
				for (int p = j; p < nums1.length && nums2.length - l + nums1.length - p >= k; p ++)
					idx1 = nums1[p] > nums1[idx1] ? p : idx1;
				for (int q = l; q < nums2.length && nums2.length - q + nums1.length - i >= k; q ++)
					idx2 = nums2[q] > nums2[idx2] ? q : idx2;
				int m1 = idx1 < nums1.length ? nums1[idx1] : -1, m2 = idx2 < nums2.length ? nums2[idx2] : - 1;
				...
				if (max >= res[i]) {
					...
					if (idx1 < nums1.length && nums1[idx1] == max) {
						next.offer(idx1 + 1);
						next.offer(l);
					}
					if (idx2 < nums2.length && nums2[idx2] == max) {
						next.offer(j);
						next.offer(idx2 + 1);
					}
				}
	...
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(new Solution().maxNumber(nums1, nums2, 500)));
	}
